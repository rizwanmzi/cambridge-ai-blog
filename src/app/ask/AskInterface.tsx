...
/** Clean AI response content for markdown rendering */
...
export default function AskInterface() {
  ...
  return (
    <div className="flex flex-col" style={{ minHeight: "60vh" }}>
      <div className="flex-1 space-y-4 mb-6">
        ...
        {messages.map((msg, i) => (
          ...
          >
            {msg.role === "user" ? (
              ...
              <div className="max-w-[85%] bg-copper-500/[0.04] border border-copper-500/10 rounded-2xl rounded-bl-md px-4 py-3">
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="text-copper-400 text-[10px] ai-pulse">&#10022;</span>
                  <span className="text-[11px] font-medium text-copper-300/70">AI</span>
                </div>
                <div className="text-sm text-[rgba(255,255,255,0.75)] leading-relaxed prose prose-invert prose-sm max-w-none prose-p:my-2 prose-headings:text-white prose-headings:font-semibold prose-headings:text-[13px] prose-strong:text-copper-300 prose-ul:my-2 prose-li:my-0.5 prose-a:text-copper-300 prose-blockquote:border-copper-500/30 prose-blockquote:text-[rgba(255,255,255,0.6)]">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>
                    {cleanContent(msg.content)}
                  </ReactMarkdown>
                </div>
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-3 pt-2.5 border-t border-copper-500/10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {msg.sources.map((src, j) => (
                        <Link
                          ...
                          className="inline-flex items-center gap-1 text-[11px] text-copper-300/70 hover:text-copper-300 bg-copper-500/5 hover:bg-copper-500/10 border border-copper-500/10 px-2.5 py-1 rounded-full transition-all duration-200"
                        >
        ...
        {loading && (
          <div className="flex justify-start animate-fade-up">
            <div className="bg-copper-500/[0.04] border border-copper-500/10 rounded-2xl rounded-bl-md px-4 py-3">
              <div className="flex items-center gap-2">
                <span className="text-copper-400 text-[10px] ai-pulse">&#10022;</span>
                <span className="text-[11px] font-medium text-copper-300/70">AI</span>
                <div className="flex items-center gap-1 ml-1">
                  <div className="w-1.5 h-1.5 rounded-full bg-copper-400/60 typing-dot" />
                  <div className="w-1.5 h-1.5 rounded-full bg-copper-400/60 typing-dot" />
                  <div className="w-1.5 h-1.5 rounded-full bg-copper-400/60 typing-dot" />
                </div>
              </div>
      ...
      >
        <div className="flex gap-2 items-center bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] rounded-2xl px-4 py-1 focus-within:border-copper-500/20 focus-within:ring-1 focus-within:ring-copper-500/10 transition-all duration-200">
          <input
            id="ask-question"
            name="ask-question"
            type="text"
          ...
          <button
            ...
            className="text-[13px] text-white bg-copper-500/80 hover:bg-copper-500 px-4 py-1.5 rounded-xl transition-all duration-200 disabled:opacity-30 shrink-0 font-medium"
          >
            Ask
          </button>
        </div>
